...
<template>
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="mt-8 bg-white overflow-hidden shadow sm:rounded-lg p-6">
        ...
        <h1>cobra event PRO 345</h1>
           <p v-if="$fetchState.pending">Loading....</p>
          <p v-else-if="$fetchState.error">Error while fetching events</p>
        <ul v-else>
          <!-- <li v-for="event in events" :key="event.pid">{{ event.name }}</li> -->
          <li> {{events}} </li>
        </ul>
...
export default {
    ...
    async fetch() {
      console.log('fetch cobra events....')

//this.events = await fetch('http://cobra-event-api.default.svc.cluster.local:8080/events').then(res =>
this.events = await fetch('https://api.dev-cobra.de/events').then(res => res.status) 
//this.events = await fetch(
  //      'http://localhost:52597/events'
//      ).then(res => res.status) 
      // axios
      // .get('http://cobra-event-api.myakscluster8439.svc.cluster.local:8080/events')
